...
# Phase 4 — Better Analytics (COMPLETED 2026-08-12)

## Scope

Minimal deterministic analytics insight added to the Dashboard. One new card: "7 ngày vừa qua".

No new charts, KPIs, navigation, pages, or AI calls.

## Implementation

- **Domain logic:** `src/features/statistics/utils/seven-day-insight.ts` — rolling 7-day windows, accuracy comparison, deterministic insight selection
- **Server loader:** `src/features/statistics/server/load-seven-day-insight.ts` — single RLS-gated query on `daily_learning_records` for 14 days
- **UI component:** `src/features/statistics/components/seven-day-insight-card.tsx` — placed between learning status and monthly activity on Dashboard
...
- 100% deterministic (zero Gemini calls)
- Uses existing `daily_learning_records` table (no migration)
- Reuses canonical `dateInTimezone` timezone convention and `accuracy()` percentage definition
