...
export default async function ProfilePage({
  searchParams,
}: Readonly<{ searchParams: Promise<{ tab?: string | string[]; month?: string | string[] }> }>) {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl p-4 sm:p-8">
      ...
      <SectionTabs
        ...
        current={tab}
        items={[
          { value: "profile", label: "Hồ sơ", href: "/profile?tab=profile" },
          { value: "statistics", label: "Thống kê", href: "/profile?tab=statistics" },
          { value: "settings", label: "Cài đặt", href: "/profile?tab=settings" },
        ]}
      />
      {tab === "statistics" ? (
