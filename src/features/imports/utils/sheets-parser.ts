...
function gridCellToString(cell: unknown): string {
  if (!cell || typeof cell !== "object") return "";
  const value = (cell as { userEnteredValue?: Record<string, unknown> }).userEnteredValue;
  if (!value) return "";
...
// Parses a spreadsheets.get response (includeGridData=true) for one sheet,
// selecting the tab by numeric sheetId — the only way to read tabs whose names
// Google's A1 range parser rejects (e.g. names containing `[` or `]`).
export function parseGridDataSheet(
  json: unknown,
  ...
  const sheet = root.sheets?.find((s) => s.properties?.sheetId === targetSheetId);
  if (!sheet) return null;

  const rowData = sheet.data?.[0]?.rowData ?? [];
  const rows: string[][] = rowData.map((row) =>
    (row.values ?? []).map((cell) => gridCellToString(cell)),
  );

  // Label of a column = its first non-empty cell (same rule as parseHeaderScan).
...
// Extracts selected columns from a full-sheet grid, matching parseColumnBodies:
// headers come from row 2 (the first data row) and bodies from row 3 onward.
export function extractColumnsFromGrid(
  gridRows: string[][],
  columns: number[],
): ColumnBodiesResult {
  ...
  for (const colIdx of columns) {
    headers[colIdx] = gridRows[1]?.[colIdx] ?? "";
    for (let gridRow = 2; gridRow < gridRows.length; gridRow += 1) {
      const value = gridRows[gridRow]?.[colIdx] ?? "";
      const targetRow = gridRow - 2;
