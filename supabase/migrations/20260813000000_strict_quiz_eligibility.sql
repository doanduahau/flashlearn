-- Strict manual-Quiz eligibility + sub-10 "Tất cả N" support.
--
-- The manual source-scoped RPC previously:
--  1. ordered all modes coverage-first, so "wrong_answers" could select an
--     uncovered-but-never-wrong card ahead of a covered-wrong card; and
--  2. required at least 10 questions, so "Tất cả N" could not start for N < 10.
--
-- This migration makes "never_tested" (Chưa làm) and "wrong_answers" (Câu sai)
-- STRICT: their eligible pool is filtered to uncovered / wrong-history cards
-- only and never backfilled, while "balanced" and "pure_random" keep the whole
-- valid source pool. It also lowers the manual minimum to one question so
-- "Tất cả N" works for 1 <= N < 10.
--
-- Smart Review and New Cards are unaffected: they use the explicit-card
-- primitive (create_quiz_session_from_card_ids) through their own wrappers,
-- never this source-scoped function.
...
begin
  ...
  -- Strict pool size: Chưa làm / Câu sai never backfill with covered or
  -- never-wrong cards. The server enforces this independently of the client.
