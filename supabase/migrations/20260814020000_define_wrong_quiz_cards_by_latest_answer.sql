-- A card belongs to the strict manual-Quiz "wrong answers" pool only when
-- its latest answered question in a completed Quiz session is incorrect.
-- Keep the established ownership, coverage, and source-selection boundaries.
...
begin
  ...
  if p_mode = 'never_tested' then
    ...
    -- answered_at is canonical; quiz_questions.id makes ties deterministic.
    select count(*) into v_strict_count
    from public.flashcards f
    where f.id = any(v_scope_card_ids)
      and (select q.is_correct = false
           from public.quiz_questions q
           join public.quiz_sessions s on s.id = q.session_id
           where q.flashcard_id = f.id
  ...
  for v_card in
    -- Use the same deterministic latest-answer rule as the strict count.
    with latest_answers as (
      select distinct on (q.flashcard_id) q.flashcard_id, q.is_correct
      from public.quiz_questions q
      join public.quiz_sessions s on s.id = q.session_id
      where q.user_id = v_user_id
        and q.answered_at is not null
        and s.completed_at is not null
      order by q.flashcard_id, q.answered_at desc, q.id desc
    ), eligible as (
      select f.id, f.front, f.back,
        ...
        coalesce(latest_answers.is_correct = false, false) as has_latest_wrong,
      ...
      from public.flashcards f
      left join latest_answers on latest_answers.flashcard_id = f.id
      ...
      where f.id = any(v_scope_card_ids)
      group by f.id, f.front, f.back, latest_answers.is_correct
    )
    select * from eligible
    where (p_mode <> 'never_tested' or has_quiz_coverage = false)
      and (p_mode <> 'wrong_answers' or has_latest_wrong)
    order by
