...
select * from public.commit_flashcard_import('  Vietnamese  ', '[{"front":"Xin chào","back":"Hello"},{"front":"Cảm ơn","back":"Thanks"}]'::jsonb,'70000000-0000-4000-8000-000000000001','manual',0,0,false);
...
select throws_ok($$select * from public.commit_flashcard_import(' ', '[{"front":"a","back":"b"}]','70000000-0000-4000-8000-000000000002','manual',0,0,false)$$, '22023', NULL, 'blank name rejected');
select throws_ok($$select * from public.commit_flashcard_import('Bad', '[{"front":"","back":"b"}]','70000000-0000-4000-8000-000000000003','manual',0,0,false)$$, '22023', NULL, 'blank card field rejected');
select throws_ok($$select * from public.commit_flashcard_import('Too many', (select jsonb_agg(jsonb_build_object('front', i::text, 'back', i::text)) from generate_series(1,2001) i),'70000000-0000-4000-8000-000000000004','manual',0,0,false)$$, '22023', NULL, 'more than 2000 cards rejected');
...
reset role;
set local role anon;
select throws_ok($$select * from public.commit_flashcard_import('Anon', '[{"front":"a","back":"b"}]','70000000-0000-4000-8000-000000000005','manual',0,0,false)$$, '42501', NULL, 'anon cannot execute RPC');
