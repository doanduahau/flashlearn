...
select is(has_function_privilege('authenticated','public.install_catalog_set_for_user(uuid,uuid,uuid,text)','execute'),false,'browser cannot invoke service install gate');
select is(has_function_privilege('service_role','public.install_catalog_set_for_user(uuid,uuid,uuid,text)','execute'),true,'service role can invoke install gate');
select is((select already_exists from public.install_catalog_set_for_user('a2000000-0000-4000-8000-000000000001','20000000-0000-4000-8000-000000000001','a2100000-0000-4000-8000-000000000001','block')),false,'first catalog install creates a clone');
...
select is((select already_exists from public.install_catalog_set_for_user('a2000000-0000-4000-8000-000000000001','20000000-0000-4000-8000-000000000001','a2100000-0000-4000-8000-000000000002','block')),true,'repeat returns existing clone');
